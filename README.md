...
Each element of your data array must be an object with properties that contain whatever data you need to fill out the record templates. This is done by using the syntax `{{property}}` anywhere inside the template element, where `property` is the actual property name of the array element that contains the data you wish to be inserted. Below is a brief example:
```html
<div class="whatever">
	<div id="my_template">
		{{name}}<br/>{{stuff}}
...
### Hooks
The app provides hooks that allow you to add functionality at specific moments. To add your function to a hook, use `ELC_addHook("<hook name>", function, params)`. `function` is the name of a function or a function definition that will be called at a certain point determined by `<hook name>`. `params` is an array that will be passed to the `apply` method and should contain the parameters of your function, if any. The following are the valid values for `<hook name>`:
* __before_update__ - These functions will be executed prior to a list container being updated with the current sort order, newly applied filters, and/or current page selection. The value of `this` in these functions will be the list container DOM element being updated.
* __after_update__ - These functions will be executed after a list container is updated with the current sort order, newly applied filters, and/or current page selection. The value of `this` in these functions will be the list container DOM element that was updated.
* __before_template_activate__ - These functions will be executed prior to a template being activated. They will not fire if the template fails to activate because it is already active or a conflicting template is active. Note that because the list will update when the records are changed, and activating/deactivating templates changes the records, the update hooks will also always execute shortly after these hooks. The value of `this` in these functions will be an object with the template details, which consists of:
	* `this.parent` - The parent element containing all of the records defined by the template.
	* `this.template` - A copy of the template element.
	* `this.data` - The data passed into the ELC_setData function.
* __after_template_activate__ - These functions will be executed after a template is activated. They will not fire if the template fails to activate because it is already active or a conflicting template is active. Note that because the list will update when the records are changed, and activating/deactivating templates changes the records, the update hooks will also always execute shortly after these hooks. The value of `this` in these functions will be an object with the template details, see above for what those entail.
* __before_template_deactivate__ - These functions will be executed prior to a template being deactivated. They will not fire if the template was not active in the first place. Note that because the list will update when the records are changed, and activating/deactivating templates changes the records, the update hooks will also always execute shortly after these hooks. The value of `this` in these functions will be an object with the template details, see above for what those entail.
* __after_template_deactivate__ - These functions will be executed after a template is deactivated. They will not fire if the template was not active in the first place. Note that because the list will update when the records are changed, and activating/deactivating templates changes the records, the update hooks will also always execute shortly after these hooks. The value of `this` in these functions will be an object with the template details, see above for what those entail.

...
Sort order can be reversed by repeatedly clicking the sorter. If you want it to be sorted in descending order on the first click instead, then add the `data-order` attribute to the sorter with any value starting with `D` (case insensitive). Additionally, if you want the list to be sorted as soon as it loads without waiting for a click, you can add the `sort-initial` class to the sorter element.
#### Defining Field Data to Sort
The records inside your list might need to be set up with specific HTML before sorting will work. If your list container is a TABLE, then you generally do not need to do anything. When a sorting column header is clicked, the records will be sorted by the content of the TD (or TH) cells in that column according to the specifications given in the sorter. However, you can still use any of the non-TABLE methods for sorters not a part of the header row if you need to. For non-TABLE list containers, the records need to have their fields specified in order for the data to be recognized. This can be done in three ways:
...
* A descendant HTML element of the record's element can include a `data-field` attribute with the value set to the field name specified in the desired sorter. The data used for sorting will be the content of the HTML element. Unless...
* Same as above, except also include a `data-value` attribute with the `data-field` attribute. In this case, the value of this attribute will be the data used for sorting.
#### Animated Sorting
